...
const displayCategories = (cats) => {
  ...
  cats.forEach((c) => {
    ...
    // 👉 এখানে শুধু category_id বা id পাঠাও
    catDiv.onclick = () => {
...
const displayPlantDetail = (p) => {
  modalTitle.textContent = getName(p);
  modalBody.innerHTML = `
    <img src="${getImg(p)}" alt="${getName(
    p
  )}" class="w-full h-60 object-cover rounded">
    <p class="text-gray-700">${getDesc(p)}</p>
    <p class="mt-2 text-sm text-gray-500"><strong>Category:</strong> ${getCat(
      p
    )}</p>
    <p class="text-green-700 font-bold mt-2">Price: $${getPrice(p)}</p>
  `;
  modal.showModal();
};
